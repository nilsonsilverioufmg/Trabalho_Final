...
#include "mqtt_client_caminhao.h"
#include "mqtt_client_caminhao.h"
...
void tarefa_coletor_dados(BufferCircular& buffer)
{
    while (true) {
        ...
        DadosVeiculo dados = buffer.get(); // Pega os dados mais recentes
        ...
        long long timestamp_ms =
            std::chrono::duration_cast<std::chrono::milliseconds>(
                dados.timestamp.time_since_epoch()).count();

        { // Leitura protegida do estado global
            std::lock_guard<std::mutex> lock(mtx_estado);
            local_modo_auto   = g_modo_automatico;
            local_falha       = g_falha_ativa;
        ...
        // 1. LOG LOCAL
        // ------------------------------
        std::cout << "[Coletor] LOG Placa " << dados.placa
                  << " - T:" << timestamp_ms
                  << " X:" << dados.posicao_x
                  << " Y:" << dados.posicao_y
                  << " Modo:" << (local_modo_auto ? "A" : "M")
                  << " Falha:" << (local_falha ? "S" : "N")
                  << " Prox:"  << (local_alerta_prox ? "S" : "N")
                  << std::endl;

        // ------------------------------
        // 2. FORMAR JSON PARA MQTT
        // ------------------------------
        std::stringstream ss;
        ss << "{";
        ss << "\"placa\":\"" << dados.placa << "\",";
        ss << "\"x\":" << dados.posicao_x << ",";
        ss << "\"y\":" << dados.posicao_y << ",";
        ss << "\"temperatura\":" << dados.temperatura << ",";
        ss << "\"modo\":\"" << (local_modo_auto ? "A" : "M") << "\",";
        ss << "\"falha\":" << (local_falha ? "true" : "false") << ",";
        ss << "\"alerta\":" << (local_alerta_prox ? "true" : "false");
        ss << "}";

        // ------------------------------
        // 3. PUBLICAR VIA MQTT
        // ------------------------------
        mqtt_enviar_estado(ss.str());
